...
CREATE INDEX IF NOT EXISTS idx_articles_published_featured ON articles(published, featured) WHERE published = true;
CREATE INDEX IF NOT EXISTS idx_articles_published_created ON articles(published, created_at DESC) WHERE published = true;
CREATE INDEX IF NOT EXISTS idx_articles_category_published ON articles(category_id, published, created_at DESC) WHERE published = true;
...
CREATE INDEX IF NOT EXISTS idx_articles_excerpt_gin ON articles USING GIN(to_tsvector('english', excerpt));
